...
export function AddToCartButton({ className }: Readonly<AddToCartButtonProps>) {
  return (
      ...
      <Link to="packages" smooth={true} duration={500}  offset={-50}
        className={twMerge(
          className,
          "cursor-pointer relative z-10 flex h-12 w-[16em] transform items-center justify-center rounded-lg border border-none border-brand-maroon bg-brand-maroon px-4 text-xl font-bold text-white transition-all duration-100 ease-in-out hover:scale-100 active:scale-[98%]",
        )}
      >
        Add To Cart
        <img
